 ...
 * dimensions. If successful, the difference is stored in 'result'. If an
 * overflow or NaN occurs, MATRIX_CALC_ERROR is returned.
 *
...
int s21_sub_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  for (int i = 0; i < A->rows && !err; i++) {
    for (int j = 0; j < A->columns && !err; j++) {
      ...
      // Check for infinity or NaN values resulting from subtraction
      if (!isfinite(sub)) {
